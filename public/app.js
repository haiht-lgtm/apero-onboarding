...
document.addEventListener('click', e => {
  ...
  if (el && !e.metaKey && !e.ctrlKey) {
    e.preventDefault();
    navigate(el.dataset.route, { id: el.dataset.id, tab: el.dataset.tab });
  }
});
...
routes.emails = async () => {
  ...
  if (byCand.size === 0) {
    html += `<div class="bg-white rounded-xl border border-slate-200 p-10 text-center text-slate-400">Không có email khớp tìm kiếm</div>`;
  }

  for (const [cid, { name, emails }] of byCand) {
...
const renderSheetResults = async () => {
  ...
  try {
    ...
    box.innerHTML = `<div class="text-xs text-slate-500 mb-2">${d.count} kết quả</div>` + d.results.map(sheetResultCard).join('');
  } catch (e) {
...
// Tab "Hồ sơ (Sheet)" trong trang chi tiết ứng viên — tự tra theo tên đang xem
const renderSheetTab = async (c) => {
  const body = $('#tabBody');
  if (!body) return;
