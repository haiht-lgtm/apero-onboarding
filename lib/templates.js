// ═══════════════════════════════════════════════════════════════════
// Templates Data — 8 emails (M1..M8) + 5 orders + 44 checklist + 25 followup
...
const EMAIL_TEMPLATES = [
  ...
  {
    key: 'M1', milestone: 'D-7', email_type: 'candidate', day_offset: -7,
...
Team HR — APERO Technologies Group`
  ...
  {
    key: 'M2', milestone: 'D-7', email_type: 'department', day_offset: -7,
...
Team HR — APERO`
  ...
  {
    key: 'M3', milestone: 'D-5', email_type: 'department', day_offset: -5,
    receiver_setting: 'dept_it_mynth_email', receiver_label: 'IT — Cấp tài khoản',
    subject: '[ORDER] Cấp tài khoản cho NV mới - {{full_name}} - {{start_date}}',
    body: `Hi team IT,

HR cần cấp đầy đủ tài khoản nội bộ cho nhân sự mới onboard ngày {{start_date}}.
...
• Cấp bậc: {{level}}
• Ngày onboard: {{start_date}}
• Email cá nhân (gửi thông tin tài khoản): {{email}}
• SĐT: {{phone}}

🔐 CÁC TÀI KHOẢN CẦN CẤP:
1️⃣ EMAIL CÔNG TY (Gmail Workspace)
   → BẮT BUỘC cấp xong trước {{start_date}} — đây là bước nền cho mọi tài khoản khác.

2️⃣ CONFLUENCE (tài liệu nội bộ)
   • Đăng nhập bằng email công ty ở trên
   • Quyền truy cập: Space chung + Space {{department}} + Space dự án/team
   • Deadline: trước {{start_date_minus_1}}

3️⃣ MISA + USERNAME (chấm công — phối hợp PhươngHT / C&B)
   • Tài khoản MISA + Username nội bộ
   • Bốc dữ liệu Form H1211 → File chính thức H1212F
   • Kích hoạt MISA sáng ngày onboard {{start_date}}

⏰ DEADLINE TỔNG: Hoàn thành toàn bộ trước {{start_date_minus_1}}

📌 LƯU Ý:
• Thứ tự bắt buộc: Email công ty → Confluence (Confluence đăng nhập bằng email công ty)
• MISA cần phối hợp với C&B (PhươngHT) — vui lòng CC khi xử lý
...
Team HR — APERO`
  ...
  {
    key: 'M4', milestone: 'D-3', email_type: 'candidate', day_offset: -3,
    receiver_field: 'personal_email', receiver_label: 'Ứng viên',
    subject: '[APERO] Chào Mừng - Chúng Tôi Đang Chờ Đón Bạn! - {{full_name}}',
    body: `Chào {{full_name}},

Chỉ còn 3 ngày nữa {{pronoun}} sẽ chính thức gia nhập đại gia đình Apero Technologies Group! Chúng tôi rất háo hức được chào đón {{pronoun}}.
...
Team HR — Apero Technologies Group`
  ...
  {
    key: 'M5', milestone: 'D0', email_type: 'candidate', day_offset: 0,
    receiver_field: 'personal_email', receiver_label: 'Ứng viên',
    subject: '[APERO] Chào Mừng Ngày Đầu Tiên - Tài Khoản Nội Bộ - {{full_name}}',
...
Team HR — Apero Technologies Group`
  ...
  {
    key: 'M6', milestone: 'D+7', email_type: 'candidate', day_offset: 7,
    receiver_field: 'personal_email', receiver_label: 'Ứng viên',
...
Team HR — Apero Technologies Group`
  ...
  {
    key: 'M7', milestone: 'D+30', email_type: 'candidate', day_offset: 30,
    receiver_field: 'personal_email', receiver_label: 'Ứng viên',
...
Team HR — Apero Technologies Group`
  ...
  {
    key: 'M8', milestone: 'D+60', email_type: 'candidate', day_offset: 60,
    receiver_field: 'personal_email', receiver_label: 'Ứng viên',
...
// Keys O1-O5 giữ nguyên mapping content để không phá state + cascade (M3→O2,O3,O4)
const DEPT_ORDERS_TEMPLATE = [
  { key: 'O4', milestone: 'D-5', day_offset: -5, deadline_offset: 0, order_type: 'MISA + Username', receiver: 'C&B (PhươngHT)', content: 'Tài khoản MISA, Username nội bộ, bốc dữ liệu Form H1211 → H1212F' },
...
const CHECKLIST_TEMPLATE = [
  ...
  { milestone: 'D-5', task_name: '📤 Báo C&B (PhươngHT) bốc dữ liệu Form sang File chính thức', assignee: 'HR → C&B', auto_done_when: { type: 'email_sent', key: 'M3' } },
  { milestone: 'D-5', task_name: '📨 Gửi email order cấp tài khoản nội bộ cho IT (email + Confluence + MISA)', assignee: 'HR → IT', auto_done_when: { type: 'email_sent', key: 'M3' } },
  ...
  { milestone: 'D-5', task_name: '💼 C&B cấp xong tài khoản MISA + Username', assignee: 'C&B (PhươngHT)', auto_done_when: { type: 'order_processed', key: 'O4' } },
  // D-3 (3)
  ...
  { milestone: 'D0', task_name: '💼 Báo PhươngHT kích hoạt tài khoản MISA', assignee: 'HR → C&B', auto_done_when: { type: 'order_processed', key: 'O4' } },
