...
// Sinh 8 emails cho 1 candidate. customTemplates tùy chọn để override defaults.
// Emails được sort theo scheduled_date + assign priority 1..n
function generateEmails(candidate, deptEmails = {}, customTemplates = null) {
  const vars = buildVars(candidate);
  const templates = customTemplates || EMAIL_TEMPLATES;
  ...
  // Sort: scheduled_date asc, rồi seq (thứ tự định nghĩa trong template)
  emails.sort((a, b) => {
  ...
  // Assign priority 1..n
  emails.forEach((e, i) => { e.priority = i + 1; });
  return emails;
}
...
function generateChecklist(candidate) {
  return CHECKLIST_TEMPLATE.map((it, idx) => ({
    ...
    auto_done_when: it.auto_done_when || null
  }));
}
