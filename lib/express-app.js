...
// Compute display_status cho TẤT CẢ emails của 1 candidate theo chain logic:
//   moc 1 → pending (luôn)
//   moc N (N>1):
//     - prev sent      → pending
//     - prev pending   → warning
//     - prev warning   → locked
//     - prev locked    → locked
...
function computeChainStatuses(emails, state) {
  ...
  for (const e of emails) {
    ...
    if (s.sent) status = 'sent';
      ...
      else if (prevStatus === 'pending') status = 'warning';
      else if (prevStatus === 'warning' || prevStatus === 'locked') status = 'locked';
      else status = 'pending';
    }
