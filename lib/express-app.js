...
const { sendEmail, signatureHtml, bodyToHtml } = require('./email');
...
// Cổng DUYỆT: state['approval:gate'].approved === true → đã duyệt
function isApproved(state) {
  return !!(state && state['approval:gate'] && state['approval:gate'].approved);
}

// Compute display_status cho TẤT CẢ emails của 1 candidate theo chain logic STRICT:
//   moc 1 → pending (luôn — email lấy thông tin, không cần duyệt)
//   moc N (N>1):
//     - CHƯA DUYỆT   → await_approval (khóa, chờ HR bấm DUYỆT)
//     - prev sent     → pending  (sẵn sàng gửi)
//     - prev khác     → locked   (strict sequential)
//   sent/failed override (state actual)
function computeChainStatuses(emails, state) {
  // emails đã sort theo priority asc (1, 2, 3, ...)
  const approved = isApproved(state);
  const statuses = new Map(); // template_key → status
  for (const e of emails) {
    const s = state[`email:${e.template_key}`] || {};
    let status;
    if (s.sent) status = 'sent';
    else if (s.status === 'failed') status = 'failed';
    else if (e.priority === 1) status = 'pending';
    else if (!approved) status = 'await_approval';
    else {
      const prev = emails.find(x => x.priority === e.priority - 1);
      const prevStatus = prev ? statuses.get(prev.template_key) : null;
      if (prevStatus === 'sent') status = 'pending';
      else status = 'locked';
    }
    statuses.set(e.template_key, status);
  }
  return statuses;
}

// Merge state vào generated email object
async function emailWithState(email, state, allEmailsForCandidate, chainStatuses) {
  const k = `email:${email.template_key}`;
  const s = state[k] || {};
  // Compute chain status nếu chưa có (lazy fallback)
  if (!chainStatuses && allEmailsForCandidate) {
    chainStatuses = computeChainStatuses(allEmailsForCandidate, state);
  }
  const display_status = chainStatuses
    ? chainStatuses.get(email.template_key)
    : (s.sent ? 'sent' : 'pending');
  return {
    ...email,
    id: `${email.candidate_id}-${email.template_key}`,
    approved: isApproved(state) ? 1 : 0,
    sent: s.sent ? 1 : 0,
...
// Cổng DUYỆT — bật/tắt mở khóa chuỗi email order cho 1 ứng viên
app.put('/api/candidates/:id/approval', async (req, res) => {
  try {
    const c = await store.getCandidate(req.params.id);
    if (!c) return res.status(404).json({ error: 'Not found' });
    const approved = !!req.body.approved;
    await store.setStateItem(c.id, 'approval', 'gate', {
      approved,
...
app.get('/api/candidates/:id/emails', async (req, res) => {
  try {
    ...
    const chain = computeChainStatuses(emails, state);
    const out = await Promise.all(emails.map(e => emailWithState(e, state, emails, chain)));
    res.json(out);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/emails', async (req, res) => {
  try {
    ...
    for (const c of cands) {
      ...
      const chain = computeChainStatuses(emails, state);
      for (const e of emails) {
        const m = await emailWithState(e, state, emails, chain);
    ...
    let filtered = all;
    if (req.query.status) filtered = filtered.filter(e => (e.display_status || e.status) === req.query.status);
...
async function findEmail(id) {
  ...
  if (!e) return null;
  const state = await store.getState(c.id);
  return { email: await emailWithState(e, state, emails), candidate: c };
}
...
app.get('/api/emails/:id/preview', async (req, res) => {
  try {
    ...
    const e = found.email;
    res.json({
      subject: e.subject, body: e.body, receiver: e.receiver, receiver_label: e.receiver_label,
      body_html: bodyToHtml(e.body), signature_html: signatureHtml()
    });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// Email ↔ Order mapping for cascade sync (Spec UX v1.1)
// M2 = Thiết bị (HCNS) → O1 | M3 = Email công ty (IT MyNTH) → O2
// M9 = MISA + Username (C&B QuynhNT) → O4 | M10 = Confluence (IT HùngNX) → O3
const EMAIL_TO_ORDER_KEYS = { M2: ['O1'], M3: ['O2'], M9: ['O4'], M10: ['O3'] };
const ORDER_TO_EMAIL_KEY = { O1: 'M2', O2: 'M3', O3: 'M10', O4: 'M9' };

async function cascadeEmailToOrders(candidateId, emailKey, sent, when) {
  const orderKeys = EMAIL_TO_ORDER_KEYS[emailKey];
  if (!orderKeys) return;
  for (const orderKey of orderKeys) {
    const cur = (await store.getStateItem(candidateId, 'order', orderKey)) || {};
    await store.setStateItem(candidateId, 'order', orderKey, {
      ...cur,
      email_sent: !!sent,
      email_sent_date: sent ? (cur.email_sent_date || when) : null
    });
  }
}
...
app.put('/api/emails/:id', async (req, res) => {
  try {
    const found = await findEmail(req.params.id);
    if (!found) return res.status(404).json({ error: 'Not found' });
    const { sent, is_completed } = req.body;
    const now = new Date().toISOString();
    if (sent !== undefined) {
      const cur = (await store.getStateItem(found.candidate.id, 'email', found.email.template_key)) || {};
      ...
      await cascadeEmailToOrders(found.candidate.id, found.email.template_key, sent, now);
    }
...
app.post('/api/emails/:id/send', async (req, res) => {
  try {
    ...
    try {
      ...
      await cascadeEmailToOrders(c.id, e.template_key, true, new Date().toISOString());
      res.json({ ok: true });
    } catch (sendErr) {
      await store.setStateItem(c.id, 'email', e.template_key, {
...
app.get('/api/dashboard/stats', async (req, res) => {
  try {
    ...
    let pendingOrders = 0;
    let overdueChecks = 0;
    let checklistDoneToday = 0;
    let checklistTotalToday = 0;
    const upcoming = [];
    ...
    for (const c of cands) {
      ...
      // checklist overdue + count today's tasks (done/total)
      const checklist = timeline.generateChecklist(c);
      for (const it of checklist) {
        const s = state[`checklist:${it.item_index}`];
        if (s?.is_skipped) continue;
...
// Gửi thử email nhắc deadline NGAY (không chờ cron 8h sáng)
app.post('/api/reminders/run', async (req, res) => {
  try {
    const result = await cronRunner.sendDueReminders({ source: 'manual' });
    res.json(result);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/docs', (req, res) => {
  res.json({
    ...
    drive: [
      ...
      { label: '� Google Sheet Onboarding', purpose: 'Danh sách/hướng dẫn onboarding', url: 'https://docs.google.com/spreadsheets/d/1nJGX480Z2M4MpUPAiUpB5VM75LpzaES8LPf5IxbZ-WE/edit?gid=1783420024' },
    ...
    people: [
      ...
      { role: 'QuynhNT (C&B)', name: 'C&B', responsibility: 'Cấp tài khoản MISA + Username + Bốc dữ liệu Form' },
...
// ═══════════════════════════════════════════════════════════════════
// TRA CỨU HỒ SƠ từ Google Sheet (chỉ đọc)
// ═══════════════════════════════════════════════════════════════════
app.get('/api/sheet/status', (req, res) => {
  res.json({ configured: sheets.isConfigured(), url: sheets.sheetUrl() });
});

app.get('/api/sheet/search', async (req, res) => {
  const q = (req.query.q || '').toString().trim();
  if (!sheets.isConfigured()) {
    return res.status(503).json({ error: 'Chưa kết nối Google Sheet. Cần đưa file google-credentials.json vào (xem HUONG-DAN-KET-NOI-GOOGLE-SHEET.md).' });
  }
  if (q.length < 2) {
    return res.status(400).json({ error: 'Nhập ít nhất 2 ký tự để tìm.' });
  }
  try {
    const results = await sheets.searchByName(q);
    res.json({ results, count: results.length });
  } catch (e) {
    const map = {
      SHEET_NOT_CONFIGURED: 'Chưa kết nối Google Sheet.',
      HEADER_NOT_FOUND: 'Không tìm thấy dòng tiêu đề trong Sheet (cần có cột "Họ và tên" và "Trạng thái").',
      NAME_COLUMN_NOT_FOUND: 'Không tìm thấy cột "Họ và tên" trong Sheet.'
    };
    let msg = e.message || 'Lỗi đọc Sheet';
    if (map[msg]) msg = map[msg];
    else if (msg.startsWith('AUTH_FAILED')) msg = 'Xác thực Google thất bại — kiểm tra lại file chìa khóa (google-credentials.json).';
    else if (msg.includes('PERMISSION') || msg.includes('403')) msg = 'Tài khoản robot chưa được chia sẻ quyền Xem file Sheet. Nhờ chủ file chia sẻ cho email service account.';
    console.error('[sheet/search]', e.message);
    res.status(500).json({ error: msg });
  }
});

// ─── Cảnh báo Onboarding (đọc file Apero Onboarding) ───
app.get('/api/onboarding/upcoming', async (req, res) => {
  if (!sheets.isConfigured()) {
    return res.json({ configured: false, url: sheets.onboardingUrl(), rows: [] });
  }
  try {
    const raw = await sheets.getOnboardingRows();
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const rows = raw.map(r => {
      let daysUntil = null;
      if (r.startISO) {
        const d = new Date(r.startISO + 'T00:00:00');
        daysUntil = Math.round((d - today) / 86400000);
      }
      return { ...r, daysUntil };
    });
    // Sắp xếp: có ngày parse được trước (gần nhất lên đầu), không parse được xuống cuối
    rows.sort((a, b) => {
    ...
    res.json({ configured: true, url: sheets.onboardingUrl(), rows });
  } catch (e) {
    let msg = e.message || 'Lỗi đọc Sheet';
    if (msg.includes('ONB_HEADER') || msg.includes('ONB_STARTDATE')) msg = 'Không tìm thấy cột "Ngày nhận việc" trong file Apero Onboarding.';
    else if (msg.startsWith('AUTH_FAILED')) msg = 'Xác thực Google thất bại — kiểm tra file chìa khóa.';
    else if (msg.includes('PERMISSION') || msg.includes('403')) msg = 'Tài khoản robot chưa được chia sẻ quyền Xem file Apero Onboarding.';
    console.error('[onboarding/upcoming]', e.message);
    res.status(500).json({ error: msg, url: sheets.onboardingUrl() });
  }
});
