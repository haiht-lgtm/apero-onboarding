  ...
  try {
    ...
    const enriched = await Promise.all(list.map(async c => {
      ...
      for (const it of checklist) {
        ...
        if (s?.is_skipped) continue; // bỏ qua items đã xóa
        totalActiveTasks++;
        if (s?.is_done) done++;
      }
      return {
    ...
    for (const item of items) {
      try {
        ...
        if (item.email_sent !== undefined) {
          ...
          if (item.email_sent && !cur.email_sent_date) next.email_sent_date = new Date().toISOString();
        }
        if (item.processed !== undefined) {
          next.processed = !!item.processed;
          if (item.processed && !cur.processed_date) next.processed_date = new Date().toISOString();
        }
...
app.get('/api/dashboard/stats', async (req, res) => {
  try {
    ...
    for (const c of cands) {
      ...
      // checklist overdue — skip cả items đã skip (đã xóa từ UI)
      const checklist = timeline.generateChecklist(c);
      for (const it of checklist) {
        const s = state[`checklist:${it.item_index}`];
        if (s?.is_skipped) continue; // bỏ qua items đã xóa
      ...
      // upcoming 7 days — đếm progress chỉ trên items chưa skip
