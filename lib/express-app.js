...
// allEmailsForCandidate (optional): all emails of same candidate, sorted by priority
//   → used to compute display_status (sent/pending/warning/locked/failed)
async function emailWithState(email, state, allEmailsForCandidate) {
  const k = `email:${email.template_key}`;
  const s = state[k] || {};
  const today = todayStr();
  let display_status; // sent | pending | warning | locked | failed
  if (s.sent) display_status = 'sent';
  else if (s.status === 'failed') display_status = 'failed';
  else if (email.scheduled_date && email.scheduled_date > today) display_status = 'locked';
  else {
    // scheduled_date <= today, chưa sent
    if (email.priority === 1) display_status = 'pending';
    else if (allEmailsForCandidate) {
      const prev = allEmailsForCandidate.find(e => e.priority === email.priority - 1);
      if (prev) {
        const prevState = state[`email:${prev.template_key}`] || {};
        display_status = prevState.sent ? 'pending' : 'warning';
...
app.get('/api/candidates/:id/emails', async (req, res) => {
  try {
    ...
    const out = await Promise.all(emails.map(e => emailWithState(e, state, emails)));
    res.json(out);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
  ...
  try {
    ...
    for (const c of cands) {
      ...
      for (const e of emails) {
        const m = await emailWithState(e, state, emails);
